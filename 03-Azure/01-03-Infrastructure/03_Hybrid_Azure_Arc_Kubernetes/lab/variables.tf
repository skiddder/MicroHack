...
variable "onprem_resources" {
  ...
  default     = ["francecentral", "germanywestcentral", "northeurope", "uksouth"]
}
...
variable "vm_size" {
  ...
  default     = "Standard_D4ds_v6" # For arc-enabled Managed SQL Instances, AMD cores are a hard requirement at the time of creating this microhack, so scale-up if required, but stick to AMD-based SKUs
}
